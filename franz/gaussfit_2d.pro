...
;
	...
	if a(3) ne 0.0 then ZZ = sqrt((X-A(1))^2+(Y-A(2))^2)/A(3) $	;GET Z
	else ZZ= 10.

	EZ = EXP(-ZZ^2)*(abs(zz) le 5.) ;GAUSSIAN PART IGNORE SMALL TERMS

	F = A(0)*EZ + A(4) +A(5)*X + A(6)*Y +A(7)*X*X +A(8)*Y*Y		;FUNCTIONS.

	IF N_PARAMS(0) LE 2 THEN RETURN ;NEED PARTIAL?
;
	s=size(zz)
	PDER = FLTARR(n_elements(zz),9) ;YES, MAKE ARRAY.

	PDER(0:*,0) = EZ		;COMPUTE PARTIALS
	if a(3) ne 0. then PDER(0:*,1) = A(0) * EZ * ZZ/A(3)
	PDER(0:*,2) = PDER(*,1)  
	PDER(0:*,3) = PDER(*,1)*ZZ
	PDER(0:*,4) = 1.
	PDER(0:*,5) = 1.*X
...
Function Gaussfit_2D, z, a
;+
; NAME:
;	GAUSSFIT
;
; PURPOSE:
; 	Fit the equation y=f(x,y) where:
;
; 		F(x,y) = A0*EXP(-z/2) + A4 + A5*x + A6*y + A7*x^2 + A8*y^2
; 			and
;		z=( (x-A1)^2 + (y-A2)^2 )/A3^2
;
;	A0 = height of exp, A1,A2 = center of exp, A3 = sigma (the width).
;	A4 = constant term, A5,A6 = linear term in X and Y, A7,A8 = quadratic 
;	term in X and Y .
;
; CATEGORY:
;	?? - fitting
...
a = [z(ix,iy), x(ix,iy), y(ix,iy), w , m, c(0), c(1) ,0 ,0] ;estimates
!c=0				;reset cursor for plotting

return,curvefit_2(z,replicate(1.,N_ELEMENTS(Z)),a,sigmaa, $
		function_name = "GAUSS_FUNCT_2D") ;call curvefit
