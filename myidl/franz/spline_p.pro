...
;	Y = array of ordinate values,1-3D, the last ordinate values
;		 corresponding to X.
;	T = vector of abcissae values for which ordinate is desired.
;		Elements of T MUST be monotonically increasing.
;
; OUTPUTS:
;	Result = miltidimentional array of interpolated ordinates.  
...
;	Change to multidimention:	Z. Yi, UiO, Jan., 1991 
; Example:
...
if n_params(0) lt 5 then sigma = 1.0 else sigma = sigma > .001	;in range?
n = n_elements(x) < n_elements(y)
;
if n le 1 then message, 'X and Y must be arrays.'


dim=size(y)	&	dim=dim(0)
