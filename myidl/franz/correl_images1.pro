function correl_images1, image_A, image_B, XSHIFT = x_shift,	$
			 	  	   YSHIFT = y_shift, 	$
					   XOFFSET_B = x_offset, $
					   YOFFSET_B = y_offset, $
					   REDUCTION = reducf,	$
...
;
; PROCEDURES USED:
;	nint - Nearest Integer function GEAENDERT ZU RFIX !!!!!!!!!
;
; PROCEDURE:
...
;-
	...
	if keyword_set( reducf ) then begin
		...
		xoff = rfix ( x_offset/reducf )
		yoff = rfix ( y_offset/reducf )
		xs = x_shift/reducf
		ys = y_shift/reducf
...
;						MONITOR=monitor, NUMPIX=numpix )
		...
		xoff = rfix( x_offset*Magf )
		yoff = rfix( y_offset*Magf )
...
;						MONITOR=monitor, NUMPIX=numpix )
	...
	xs = rfix( x_offset ) - x_shift
	ys = rfix( y_offset ) - y_shift
