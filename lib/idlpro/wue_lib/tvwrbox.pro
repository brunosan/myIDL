...
;       TVWRBOX
; PURPOSE:
;       Display a byte array on screen using a box for position and size.
; CATEGORY:
; CALLING SEQUENCE:
;       tvwrbox, a
; INPUTS:
...
; KEYWORD PARAMETERS:
;       Keywords: 
;         /BILINEAR uses bilinear interpolation to change array size. 
...
;-
	...
	if code eq 2 then begin			; Was abort write.
	  tvbox2, xw, yw, dxw, dyw, -1		; Erase box.
	  return
	endif
