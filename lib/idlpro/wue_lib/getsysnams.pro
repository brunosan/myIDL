...
; INPUTS:
;       evar = environ. var. (symbol in VMS) containing file name.   in 
; KEYWORD PARAMETERS:
;       Keywords: 
;         ERROR=err  0=ok, 1=no such environ. var, 2=file not found. 
; OUTPUTS:
;       n = number of elements in each list.                         out 
;       lst1, lst2, lst3 = output lists.                             out 
; COMMON BLOCKS:
; NOTES:
;       Notes: the lines in the file are returned divided into as many
;	  output lists as are requested, with the first line in the first
;	  list, second in second, .... File lines starting with * are
;	  considered comments and are ignored. This routine is useful
;	  for system dependent lists, such as postscript printers or 
;	  tape drives.  As an example for printers let the file contain: 
...
;         To use: 
;         (1) Set up a systen environmental variable, like IDL_PSPRINTERS, 
;             with the name of the above file. 
;         (2) getsysnams,'idl_psprinters', n, cmds, nams 
;             will give: n = 2, cmds = string array of print commands, 
;             nams = string array of descriptive names (good for menus). 
