...
;       LATEXON
; PURPOSE:
;       Make an encapsulated postscript file to include in LaTeX documents.
; CATEGORY:
; CALLING SEQUENCE:
;       latexon, file
; INPUTS:
...
;         (3) Repeat same plot (use /REVERSE for annexe). 
...
;             IDL_DIR:[LIB]IDLPLT_DVIPS.TEX, which defines the insertplot
;             macro. 
...
;	   or \insertimg (same args. Use output shape same as image shape). 
...
;-
	...
	if (n_params(0) lt 1) or keyword_set(hlp) then begin
	  print,' Make an encapsulated postscript file to include'+$
	    ' in LaTeX documents.'
	  print,' latexon, file'
	  ...
	  print,'   (3) Repeat same plot (use /REVERSE for annexe).'
