...
;       constant background.
;       f = a0 * exp( ((x-a1)/a2)**2/2.) + a3 + a4*x
; CATEGORY:
;       Curve fitting
; CALLING SEQUENCE:
;       GSLIN_FIT,x,y,par,nostart=nostart,yfit=yfit
;       GSLIN,x,par,f [,pder]
; INPUTS:
...
;                 input vector x.
; OUTPUTS:
;       par     - fit parameters, for use as input in  GSLIN
;       f       - fittet values for x (in GSLIN) using parameters 'par'
...
;
;
pro gslin_fit,x,y,par,nostart=nostart,yfit=yfit
on_error,2
pname='GSLIN_FIT'
...
if (keyword_set(nostart)) and (n_elements(par) ne 5) then begin
        print,mess,'you must supply a 5-element vector'
...
if not keyword_set(nostart) then begin
	...
	par(4)=(y(nelx)-y(0))/(maxx-minx)
	endif

w=fltarr(nelx+1) + 1. ; equal weights for all data
yfit=curvefit(x,y,w,par,sig,funct='gslin')
return
end
