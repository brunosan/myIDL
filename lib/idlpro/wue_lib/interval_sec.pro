...
;       The format of the date and time strings is flexable, see brk_date_time. 
...
;-
	...
	FUNCTION INTERVAL_SEC, T1, T2, help=hlp
 
	if (n_params(0) lt 2) or keyword_set(hlp) then begin
	  print,' Find time interval in seconds between two date/time strings.'
	  print,' sec = interval_sec(t1, t2)'
	  print,'    t1, t2 = date and time strings.        in'
	  print,'    sec = Time in seconds from t1 to t2.   out'
	  print,' Note: date and time strings are strings like 21-Jan-1989 14:43:03'
	  print,' The format of the date and time strings is flexable, see brk_date_time.'
	  return, -1
	endif
 
	;-------  T1: First date and time  ----------
	BRK_DATE_TIME, T1, DT, TM	; Break T1 into date and time.
	DATE2YMD, DT, Y, M, D		; Find Y, M, D.
	JD1 = YMD2JD(Y,M,D)		; Find JD.
	SEC1 = DOUBLE(SECSTR(TM))	; Time in sec.
 
	;-------  T2: Second date and time  ----------
	BRK_DATE_TIME, T2, DT, TM	; Break T2 into date and time.
	DATE2YMD, DT, Y, M, D		; Find Y, M, D.
