...
;	DMS, Nov, 1988.
;	Keyword /ynozero added at defining scales. A.Welz Wue. Aug. 1992
;-
...
while 1 do begin
	...
	if mode ne old_mode then begin
		...
		if mode then begin	;Columns?
			plot,/yno,[minv,maxv],[0,ny-1],/nodat,title='Column Profile'
			vecy = findgen(ny)
			crossx = [-tickl, tickl]*(maxv-minv)
			crossy = [-tickl, tickl]*ny
		end else begin
			plot,/yno,[0,nx-1],[minv,maxv],/nodata,title='Row Profile'
			vecx = findgen(nx)
