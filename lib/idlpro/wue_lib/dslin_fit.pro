...
;       profile (Lorentzian) with linear background.
;       f = a0 * a1 / ( (x-a1)^2 + a2^2/4.)  + a3 + a4*x
; CATEGORY:
;       Curve fitting
; CALLING SEQUENCE:
...
;                 input vector x.
; OUTPUTS:
;       par     - fit parameters, for use as input in  DSLIN
;       f       - fittet values for x (in DSLIN) using parameters 'par'
...
;
;
pro dslin_fit,x,y,par,nostart=nostart,yfit=yfit
on_error,2
pname='DSLIN_FIT'
...
yfit=curvefit(x,y,w,par,sig,funct='dslin')
return
end
