...
;       FITSPUTI
; PURPOSE:
;       Put an integer keyword and value into a FITS header.
; CATEGORY:
; CALLING SEQUENCE:
;       fitsputi, hdr, key, val, [comm]
; INPUTS:
;       key = FITS keyword.                            in 
;       val = keyword integer value.                   in 
...
;-
	...
	pro fitsputi, hdr, key, val, comm, error=err, help=hlp, update=update
 
	if (n_params(0) lt 3) or keyword_set(hlp) then begin
	  print,' Put an integer keyword and value into a FITS header.'
	  print,' fitsputi, hdr, key, val, [comm]'
	  print,'   hdr = an existing FITS hdr as a string array.  in,out'
	  print,'   key = FITS keyword.                            in'
	  print,'   val = keyword integer value.                   in'
	...
	;------  Not FITS header  ---------
	print,' Error in fitsputi: invalid FITS header.'
	err = 1
	return
...
next:	if keyword_set(update) then begin
	  ...
	  for i = 0, lh do begin
	    if k eq getwrd(hdr(i),0) then begin
	      ...
	      hdr(i) = fitstxti(k,val,comm)
	      err = 0
	      return
	    endif
	  endfor
	  print,' Error in fitsputi: keyword not found for update.'
	  err = 2
	  return
	endif
 
	;-----  not update, add new  ---------------
	new = fitstxti(key, val, comm)
