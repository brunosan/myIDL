...
; 	T. Rimmele, Nov 1991 ---changed fitsdata to return header and named it
;			     ---fitsread
;-
 
	pro fitsread2, file, h, data, set=set, number=num, one_d=one_d, $
	...
	if (n_params() lt 2) or keyword_set(hlp) then begin
	  ...
	  print,' fitsread2, file, hdr, data'
	  print,'   file = Name of FITS file.        in'
	  print,'   hdr = returned header.            out'
