...
; KEYWORD PARAMETERS:
;       Keywords: 
;         /UNEVEN means file records have a variable number of columns. 
;           Process each record by itself.  Slower. 
;	  /SIGN means return only the arithmetic sign of each number.
...
jmp:	s = s(0:i)		; Trim off un-used space.
	...
	if keyword_set(un) then begin
	  ...
	  for i=0, ny-1 do begin			; Loop thru records.
	...
	for i = 0, nx-1 do begin	; Loop thru columns.
