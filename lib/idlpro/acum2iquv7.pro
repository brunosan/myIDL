pro acum2iquv7,map_in,fileff,filecalib,display=display,$
...
if(n_elements(data) ne 8) then begin
   dcff=rfits_im(fileff(0),1,desp=desp)>0
   for j=2,8 do dcff=dcff+(rfits_im(fileff(0),j,desp=desp)>0)
   dcff=dcff/8.
   im1=(rfits_im(fileff(0),9,desp=desp)>0)-dcff
   im2=(rfits_im(fileff(0),10,desp=desp)>0)-dcff
   im3=(rfits_im(fileff(0),11,desp=desp)>0)-dcff
   im4=(rfits_im(fileff(0),12,desp=desp)>0)-dcff
...
for i=0L,npos-1,step do begin
   ...
   imdemod=mat_tel # dmod # reform(im_a,4,long(dd.naxis1)*long(dd.naxis2))
   imdemod=reform(imdemod,4,long(dd.naxis1),long(dd.naxis2))
   for j=0,3 do begin
   ...
   if(timeff1 ne timeff2) then begin
      imdemod=mat_tel # dmod # reform(im_b,4,long(dd.naxis1)*long(dd.naxis2))
