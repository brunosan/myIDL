...
;       image) ** in an IDL-structure **.  This structure also keeps all
;	comments and other parameters related to the image.
;*CATEGORY:            @CAT-#  2 11@
;	CCD Tools , FITS-Files
; CALLING SEQUENCE:
;	RDFITS,ccd_structure [,file-name] [,/EXABYTE] [,/SKIP_FILE=n] ...
;             ... [,/DELETE] [,/RESET] [,ERRFLAG=errflag]
; INPUTS:
;       ccd_structure : IDL-structure containing the image and all relevant 
;	    parameters (see description below under "OUTPUTS"). 
;           If, on input, it has the expected structure-format, and if keyword
;	    /RESET has not been set, this structure will be used again;
;	    otherwise, the procedure will request the user to enter the format
;	    for the image-array which (only 4 formats are possible:  
;	    1024x1024, 512x512, 256x256, 400x600) and for sampling parameters.
;	    Examples (no /RESET in all cases):
;		ccd=0
;		rdfits,ccd, ...   RDFITS will create a structure & will querry
;			          for parameters;
...
;	       file names; if no file is found on 1st attempt, RDFITS will try
...
;	       ExaByte-tape, the header is "reversed" but the image array is
;	       "regular"; if disk file was written by KIS_LIB procedure WRFITS,
;	       both header and image array are "regular").
...
;       /EXABYTE :  key-word; 
;	    ** If set **: 
;	       This procedure spawns a UNIX DD command to dump (and,
;              request, byte-swap) one file from ExaByte tape to disk. 
;	       If RDFITS is called for the 1st time with /EXABYTE is set (or
...
;	              it's size can be chosen by the user (see above "size of
;                     .pic"); if the original CCD-image does not fit into this
;	              array, the user will be requested to specify how to
...
jmp22: print,'Enter ExaB-device-name (e.g. nrst0)'
   ...
   exab_driv=strlowcase(strcompress(exab_driv,/remove))
;
...
;	nlte, 1993-Feb-23  error messages to unit -2
...
txt='% RDFITS: DD from ExaByte unsuccessful: no message from DD' & goto,junsucc
endif
if n_out_dd gt 0 then print,out_dd
if n_out_dd lt 2 then begin 
   txt='% RDFITS:  DD from ExaByte unsuccessful. ' & goto,junsucc
endif
if strmid(out_dd(0),0,3) eq '0+0' or strmid(out_dd(1),0,3) eq '0+0' then begin
   errflg=1 & txt='% RDFITS:  DD from ExaByte unsuccessful. ' & goto,junsucc
endif
;
return   ; successful
;
...
;
;
PRO CHECK_STRUCT,ccd_struct,neu,nx,ny,kx,ky
;
; NAME:
;	CHECK_STRUCT
; PURPOSE:
;	Checks 1st argument if appropriate structure for storing an exposure
;	(image array & header infos); if not, a new structure will be created
;	(user must enter requested format for image array); checks format
;	of original image format of exposure vs. format of array .pic and
;	requests user to enter new binning/scissoring parameters if neccessary.
;*CATEGORY:            @CAT-#  2 11@
;	AUXILLARY/CCD
; CALLING SEQUENCE:
;	CHECK_STRUCT,ccd_struct,neu,nx,ny,kx,ky
; INPUTS:
;	ccd_struct : if a valid structure for storing the exposure as created
;                    by a previous call, this structure will be used again (if
;		      possible);
;		     else, such a structure will be created.
;	          !!! may be modified on return !!!
;       neu (logical): =1:  forces ccd_structure to be re-created in any case.
;		  !!! may be modified on return !!!   
; OUTPUTS:
;       ccd_struct : valid structure for storing the exposure;
;	nx, ny    : size of array ccd_struct.pic (may be > than physical image)
...
if neu ne 0 then begin
   ...
   jmp1: print ,$
      '    Enter: b (=1024x1024), m (=512x512), r (=320x512) s (=400x600), t (256x256)'
   form_pic=''
...
;
; check size of image-array on disk (naxis1,2) <= size of .pic (nx,ny) :
...
;     user must specify (new) scissoring / binning  or other format for .pic
 ...
 FORMAT='("size of read-in image = ",i0," , ",i0," larger than ",i0," , ",i0)'$
      ,naxis1,naxis2,nx,ny 
      print,$
      'Extraction of a sub-image and/or binning or new structure-format required !'
again4:
     print,'Enter "n" (new structure) or : "s" (sub-image) "b" (binning) "sb" (both) !'
...
if n_params() le 0 then begin
 print,'Usage: RDFITS,ccd_structure [,infil] [,/EXABYTE [,/SKIP_FILE=nskip] ...'
 print,'       ... [,/DELETE] [,COMM=string]] [,/RESET] [,ERRFLAG=errflag]'
 goto,jexit
endif
;
...
;print,'nach dump_exab: ddfil0,n_out_dd,search,errflg=',ddfil0,n_out_dd,search,errflg
...
fits_hdr,unit,date,time,datemodif,proj,txt,fitsfile,id,itime,expos,bb,$
         szx,szy,nrecs
; check :
;print,'nach fits_hdr: search=',search,' id=',id
...
print,$
   ...
   ,naxis1,naxis2
;
; check ccd_struct; user must specify format if not already done
check_struct,ccd_struct,neu,nx,ny,kx,ky
;
case bb(0) of
   -9 : typ='unknown'
...
endcase
   ...
   ccd_struct.time=time    ; start time exposure
   ccd_struct.id=id
   ccd_struct.itime=itime  ; start time exposure sec since midnight
   ccd_struct.expos=expos
;
; read image array:
...
;  ***** image is smaller than ccd_struct.pic ***** :
    ccd_struct.pic=0
    ccd_struct.pic(0:naxis1-1,0:naxis2-1)=p(0)
...
;  ***** image is larger than ccd_struct.pic ***** do scissoring/binning:
   ...
   if binx gt 1 or biny gt 1 then $
      ccd_struct.pic(0:kx-1,0:ky-1) = rebin((p(0))(x0:xm,y0:ym),kx,ky) $
   else ccd_struct.pic(0:kx-1,0:ky-1)=(p(0))(x0:xm,y0:ym)
