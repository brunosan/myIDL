pro acum2iquv3,map_in,fileff,filecalib,pzero,rzero,delta,display=display,$
...
flat_tip,fileff(0),ff,dmod,zqq,zuu,zvv,poss,pendiente,data=data,time=timeff
timeff=mean(timeff)
flat_tip,fileff(1),ff2,dmod,zqq,zuu,zvv,poss,pendiente,data=data,time=timeff2
timeff2=mean(timeff2)
;for j=1,nff-1 do begin
;flat_tip2,fileff,ff,dmod,zqq,zuu,zvv,data=data  ; este suma tal cual
;   flat_tip,fileff(j),fftmp,dmod,zqq,zuu,zvv,poss,pendiente,data=data,$
;      time=timeff
;   ff=ff+fftmp
;endfor  
;ff=ff/nff 
...
;      mat_tel=vtt(n,k,date(2),date(1),date(0),time(i),theta,105.,lambda=lambda)
   ...
   endelse      
   imdemod=mat_tel # dmod # reform(im,4,long(dd.naxis1)*long(dd.naxis2))
   ffdemod=mat_tel # dmod # reform(ff2,4,long(dd.naxis1)*long(dd.naxis2))
   imdemod=reform(imdemod,4,dd.naxis1,dd.naxis2)
   ffdemod=reform(ffdemod,4,dd.naxis1,dd.naxis2)
   for j=0,3 do begin
      dum=reform(imdemod(j,i3:i4,j3:j4))
      dum=desplaza(dum,poss)
      imdemod(j,i3:i4,j3:j4)=dum
      dum=reform(ffdemod(j,i3:i4,j3:j4))
      dum=desplaza(dum,poss)
      ffdemod(j,i3:i4,j3:j4)=dum
   endfor   
   im2(0,*,*)=(imdemod(0,i1:i2,j1:j2)+imdemod(0,i3:i4,j3:j4))/2.
   im2(1:3,*,*)=(imdemod(1:3,i1:i2,j1:j2)-imdemod(1:3,i3:i4,j3:j4))/2.
   ffd2(0,*,*)=(ffdemod(0,i1:i2,j1:j2)+ffdemod(0,i3:i4,j3:j4))/2.
   ffd2(1:3,*,*)=(ffdemod(1:3,i1:i2,j1:j2)-ffdemod(1:3,i3:i4,j3:j4))/2.
   if(filt eq 1) then begin
      for j=0,j2-j1 do for k=0,3 do im2(k,*,j)=median(reform(im2(k,*,j)),3)
      for j=0,i2-i1 do for k=0,3 do im2(k,j,*)=median(reform(im2(k,j,*)),3)
      x=findgen(i2-i1+1)
      for j=0,j2-j1 do begin
         esp=reform(im2(0,*,j))
         im2(0,*,j)=esp-ajusta_seno(x,esp,12)+mean(ajusta_seno(x,esp,12))
      endfor
      for j=1,3 do im2(j,*,*)=filtra_tip(reform(im2(j,*,*)))
