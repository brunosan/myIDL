pro acum2iquv11,map_in,fileff,filecalib,display=display,$
...
if(n_elements(data) ne 8) then begin
   dcff=rfits_im(fileff(0),1)>0
   for j=2,8 do dcff=dcff+rfits_im(fileff(0),j)>0
   dcff=dcff/8.
...
for i=0L,npos-1,step do begin
   ...
   for j=0,3 do begin
      im_a(j,*,*)=(corrige(rfits_im2(datos,dd,4*i+j+9,desp=desp)>0)-dc)/ff1(j,*,*)
      im_b(j,*,*)=(corrige(rfits_im2(datos,dd,4*i+j+9,desp=desp)>0)-dc)/ff2(j,*,*)
   endfor   
