pro flat_tip5,file,ff,r,espim,pos,pendiente,data=data,desp=desp   
...
espim=fltarr(4,i2-i1+1,j2-j1+1)
tam=size(ff)
...
if(pendiente ne 0) then begin
   ...
   esp=esp/tam(3)
   for j=0,tam(3)-1 do espim(0,*,j)=interpol(esp,xx,xx-j*pendiente)
   for j=1,3 do espim(j,*,*)=espim(0,*,*)   	  
endif else begin
   for j=0,tam(2)-1 do esp(j)=median(iffm(j,*))
   cont=con_fit(esp,2)
   esp=esp*mean(cont)/cont
   for j=0,tam(3)-1 do espim(0,*,j)=esp
   for j=1,3 do espim(j,*,*)=espim(0,*,*)   	  
endelse   
