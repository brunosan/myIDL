pro flat_tip3,file,ff,r,pos,pendiente,data=data,desp=desp   
...
if(pendiente ne 0) then begin
   for j=0,tam(3)-1 do begin
      esp=esp+interpol(ff(*,j),xx,xx+j*pendiente)
   endfor   
   esp=esp/tam(3)

   for j=0,tam(1)-1 do begin
      for k=0,tam(3)-1 do begin
         ff(j,*,k)=ff(j,*,k)/interpol(esp,xx,xx-k*pendiente)
      endfor
   endfor
endif else begin
   for j=0,tam(2)-1 do esp(j)=median(ff(j,*))
   cont=con_fit(esp,2)
   esp=esp*mean(cont)/cont
   for j=0,tam(3)-1 do ff(*,j)=ff(*,j)/esp
