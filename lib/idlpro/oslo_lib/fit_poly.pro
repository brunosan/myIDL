...
;	every spatial point in the images we shall write:
;
;	IDL> x = FINDGEN(m)		;The wavelengths.
...
ON_ERROR,2
	...
	CASE 1 OF
		s(0) EQ 1: coeff = FLTARR(n+1,/nozero)
		s(0) EQ 2: coeff = FLTARR(s(1),n+1,/nozero)
		ELSE: coeff = FLTARR(s(1),s(2),n+1,/nozero)
	ENDCASE
