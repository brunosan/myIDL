pro calib_slit4,file,ff,dmod,delta,eps,data=data,plot=plot,desp=desp,lambda=lambda
...
for i=0,npos-1 do begin
   ...
   for j=0,3 do begin
      im=((rfits_im2(datos,dd,4*i+j+9,desp=desp,/badpix)>0)-dc)/ff(j,*,*)
      im=median(im,3)
...
; BEGIN get rpos (NEW MAY 2007)

ang=findgen(901)*0.1-45
sig=fltarr(901)

for jjj=0,900 do begin

   for j=0,npos-1 do begin
      luzin(j,*)=retarder(thret(j)+rzero,delta)# $
          impolariz2(q,thpol(j)+ang(jjj))#uno
   endfor   
...
;      print,slit,coef(1)
   ...
   sig(jjj)=std(haz(z,*)-luzfit(z,*))
endfor

pzero=ang(where(sig eq min(sig)))

; END get rpos (NEW MAY 2007)
...
;   stop
   ...
   for j=0,3 do begin
      z=where(peso(*,j) eq 1) 
;*********** esto es nuevo************8
 ;      coef=lstsqfit(luzin(z,*),haz1(z,j)+haz2(z,j),yfit)
 ;      mm(j,0,slit)=coef(0,0)
 ;      dmm(j,0,slit)=coef(0,1)/mm(0,0,slit)
;*********** hasta aqui************8
      mm(j,0,slit)=mean((haz1(z,j)+haz2(z,j))/(haz1(z,0)+haz2(z,0)))
      dmm(j,0,slit)=stdev((haz1(z,j)+haz2(z,j))/(haz1(z,0)+haz2(z,0)))
   endfor  
;   mm(*,0,slit)=mm(*,0,slit)/mm(0,0,slit)
