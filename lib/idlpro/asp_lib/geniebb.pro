pro geniebb, im1, im2, im3, im4, range1, range, ct
;+
;
; routine to generate for publication the images for op 5 of intensity,
; error in field strength, B0, and B1.  Outputs these
...
if n_params() ne 7 then begin
	print
	print, "usage:  geniebb, im1, im2, im3, im4, range1, range, ct"
	print
	print, "	Bruce's code... (see pltiebb.pro)."
	print
	return
endif
;-
...
aimage = reform(aimage,xdim,ydim)
 badcol,aimage
aimage = aimage < 1000.
whr = where(aimage gt 0)
aimage(whr) = alog10(aimage(whr))
tvscl,aimage,2*xdim,0*ydim
aimage(whr) = aimage(whr)*1000.
;  convert to integer image
im2 = intarr(xdim,ydim)
im2 = fix(aimage)
;  outline umbral regions
im2(whrumb) = -2
tvscl,im2>0,1*xdim,1*ydim
range(*,1) = [0,4000]
ct(0) = 0
im2(where(im2 lt -2)) = -1		; set bad values as 'no data'

;  plot surface value of source function B0
tmp = read_floats( dir+'a_bzero' )
 terpol,tmp,whrsv
aimage = fltarr(xdim*ydim,/nozero)
aimage(*) = -1.
aimage(sxy) = tmp
...
im3(whr) = -1
im3(whrumb) = -2
tvscl,im3>0,2*xdim,1*ydim
range(*,2) = [0,1000]
ct(0) = 2


;  plot slope of source function
tmp = read_floats( dir+'a_b1mu' )
 terpol,tmp,whrsv
aimage = fltarr(xdim*ydim,/nozero)
aimage(*) = -1.
aimage(sxy) = tmp
