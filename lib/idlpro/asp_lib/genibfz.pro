pro genibfz, im1, im2, im3, im4, range1, range, ct
;+
;
;  purpose:  generate for publication the images for op 5 of intensity,
; 	     abs(field strength), fill factor, and net polarization.
;	     Outputs these respectively in im1-im4.  range1 gives the
;	     range of x,y dimensions in arcseconds (since the op5 is
;	     near the limb, don't use megameters here).
;
;  notes:  range(2,4) gives range of color tables, 
...
if n_params() ne 7 then begin
	print
	print, "usage:  genibfz, im1, im2, im3, im4, range1, range, ct"
	print
	print, "	Bruce's code... (see pltibfz.pro)."
	print
	return
endif
;-
...
;stop
			...
			;Display abs(field strength) image.
			;
...
ct(1) = 1
			...
			;
			;Read fill factor data.
			;Display fill factor image.
			;
tmp = read_floats( dir+'a_alpha' )
aimage = fltarr(xdim*ydim,/nozero)
 terpol,tmp,whrsv
aimage(*) = -1.
aimage(sxy) = tmp
...
;tmpmid = aimage(*,50:200)
;aimage = aimage > min(tmpmid) < fmax
;  store fill factor image
im3 = intarr(xdim,ydim)
whr = where( aimage ge 0)
aimage(whr) = 1000.*(1.-aimage(whr))
im3 = fix(aimage)
im3(whrumb) = -2
...
ct(2) = 0
			...
			;Scale and move the information to the byte image.
			;Display polarization fraction image.
			;
