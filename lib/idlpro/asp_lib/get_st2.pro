...
if n_params() lt 2 then begin
	...
	print, "	    fop, lop	- first and last op numbers of maps"
	print, "	 	          (defs=use 'olist')"
	print, "	    inp, inn	- input file names of neg. and pos."
	print, "	 	          extrema (defs='614[+|-]v.st')"
	print, "	    outp, outn	- output file names of neg. and pos."
	print, "	 	          extrema (defs='614[+|-]v.st2')"
...
;	Set input St files.
;
inn_file = '614-v.st'		; negative extremum for entrance V  (def)
inp_file = '614+v.st'		; positive extremum for entrance V  (def)
...
;	Set output St files.
;
outn_file = '614-v.st2'		; negative extremum for entrance V  (def)
outp_file = '614+v.st2'		; positive extremum for entrance V  (def)
...
;	Read old St info for the map.
	...
	readf, inp_unit, smin, format=f2
	readf, inn_unit, iconfig, vttel, vttaz, tblpos, phiaz, format=f1
	readf, inn_unit, smax, format=f2
;
;	Print old St's.
	print, '-------------------------------------------------------------'
	...
	print
	print, 'OLD St''s ...'
	print, smin, format=f2
	print, smax, format=f2
;
...
;
	if set_plot then results(0, 0, 0:3, iop) = smin		; save old
	smin = transpose(invert(xnew)) # (xoldt # smin)		; calc new
	i = smin(0)	&   smin = smin / i			; normalize
	if set_plot then results(1, 0, 0:3, iop) = smin		; save new
;
	if set_plot then results(0, 1, 0:3, iop) = smax		; save old
	smax = transpose(invert(xnew)) # (xoldt # smax)		; calc new
