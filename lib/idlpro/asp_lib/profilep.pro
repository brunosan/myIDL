pro profilep, image, sx = sx, sy = sy, wsize = wsize, order = order, $
	xpos=xpos, ypos=ypos
;+
; NAME:
;	PROFILEP	(Rob's version of PROFILES with xpos/ypos.)
;
; PURPOSE:
...
;		keyword is omitted, 0 is assumed.
;
;	WSIZE:	The size of the PROFILEP window as a fraction or multiple 
;		of 640 by 512.
;
...
;
;	xpos,
;	ypos:  pixel position for window
;
; OUTPUTS:
;	No explicit outputs.
;
...
;
; EXAMPLE:
;	Create and display an image and use the PROFILEP routine on it.
;	Create and display the image by entering:
;
;		A = BYTSCL(DIST(256))
;		TV, A
;
;	Run the PROFILEP routine by entering:
;
;		PROFILEP, A
;
;	The PROFILEP window should appear.  Move the cursor over the original
...
;	DMS, Nov, 1988.
;	Rob Montgomery, Feb. 1993 - added xpos, ypos keywords.
;
;==============================================================================
;
;	Check parameters.
...
if n_params() ne 1 then begin
	print
	print, "Rob's version of PROFILES."
	print
	print, "	New Keywords"
	print, "	    xpos,ypos	 - position of window in pixels"
	print
	return
endif
;-
...
if (n_elements(xpos) eq 0) or $
   (n_elements(ypos) eq 0) then message, 'must specify xpos and ypos!'
...
while 1 do begin
	...
	if (x lt nx) and (y lt ny) and $
		...
		if first eq 0 then begin	;Erase?
			...
			xyouts,.80,.85,/norm,value,col=0,align=.5 ;Erase text
			empty
		  endif else first = 0
;;;;		value = string([x,y],format="('(',i4,',',i4,')')")
;;;;		value = strtrim(x,2) + string(y)
		value = 'X:' + strmid(x,8,4) + '  ' + $
			'Y:' + strmid(y,8,4)
		...
		if mode then begin		;Columns?
			...
			old_y1 = [ixy,ixy]
		  endelse
		xyouts,.80,.85,/norm,value,align=.5	;Text of locn
