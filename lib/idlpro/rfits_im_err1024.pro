...
while(fin eq 0) do begin
   ...
   if(pos ne -1) then desc.date=strtrim(head(pos+10:pos+29))
   ...
   if(pos ne -1) then desc.telescope=strtrim(head(pos+11:pos+26),2)

   pos=strpos(strtrim(head),'CAMERA  =')
   if(pos ne -1) then desc.camera=strtrim(head(pos+11:pos+26),2)
   ...
   if(pos ne -1) then begin
       dum1=byte(strtrim(head(pos+11:pos+26),2))
